...
public class DBScan {
  ...
  // parcourrir chacun de point de la liste 
   for(Point3D point1 :point){
    ...
    // si le point n'est pas indefini, on le met dans le clusterlabel puis on fait une sauvergarde de ses voisins
    ...
    /*on cree une pile 
     * on ajoute tous les voisins dans la pile et si le point est un bruit alors on changera son label tant que la pile n'est pas vide 
     */
...
/*try (PrintWriter pr = new PrintWriter(filename)) {
    ...
    String[][] colorList=new String[nombre][];
    ...
    for (Point3D p : point ){
      ecriture.println(p.getX() + ";" + p.getY() + ";" + p.getZ() + ";" + p.getcluster()+array[0]+","+array[1]+","+array[2]); 
    }
    ecriture.close();
  }
}
...
/* bonjour professeur mon RGB genere une boucle infinie c'est pouquoi je l'ai mis en commentaire */
  ...
  try {
    ...
    ecriture.println("x" + ";" + "y" + ";" + "z"  + ";"  + "C" + ";" + "R" + ";" + "G" + ";" + "B");
    for (Point3D p : point ){
      ecriture.println(p.getX() + ";" + p.getY() + ";" + p.getZ() + ";" + p.getcluster()); 
    }
    ecriture.close();
